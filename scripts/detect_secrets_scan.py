...
def findings(data: dict) -> list[tuple[str, dict]]:
    return [(filename, item) for filename, items in data.get("results", {}).items() for item in items]

def scan_current_tree() -> list[str]:
    return [f"árvore atual: {filename}:{item.get('line_number', '?')} — {item.get('type', 'possível segredo')}" for filename, item in findings(detect(all_files=True))]

def scan_history_paths() -> list[str]:
    problems: list[str] = []
    for raw in sorted(set(run("git", "log", "--all", "--format=", "--name-only").splitlines())):
        path = raw.strip()
        if path and path != ".env.example" and SENSITIVE_PATH_RE.search(path):
...
def scan_history_content() -> list[str]:
    ...
    for line in patch.splitlines():
        if line.startswith("diff --git a/"):
            ...
            continue
        if not current_path or EXCLUDE_RE.search(current_path) or line.startswith(("+++", "---")):
            continue
        if line.startswith(("+", "-")):
            value = line[1:]
            if not re.fullmatch(r"\s*[0-9a-f]{40,64}\s*", value, re.IGNORECASE):
    ...
    return [f"histórico: possível segredo em conteúdo alcançável — {item.get('type', 'detector desconhecido')}" for _, item in findings(data)]
